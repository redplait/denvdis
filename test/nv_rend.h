...
enum NVLType {
  ...
  Atom_sys, // EIATTR_ATOM_SYS_INSTR_OFFSETS
  ...
  Sys_call, // EIATTR_SYSCALL_OFFSETS
 ...
 public:
  ...
  void open_log(const char *of) {
   ...
   NV_ELog *m_elog = nullptr;
    ...
    std::optional<int> relax_latency(const struct nv_instr *, const NV_extracted &) const;
    bool delay_plop(const struct nv_instr *, const NV_extracted &) const;
  ...
  protected:
   ...
   const NV_field *has_rsimm(const NV_rlist *rl, const struct nv_instr *i, std::string_view *name = nullptr) const;
